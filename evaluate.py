...
def parse_args():
    parser = argparse.ArgumentParser(description="Test Evaluation for Math Problem Solving")
    ...
    parser.add_argument(
        "--sc_batch_size", type=int, default=1,
        help="Batch size for self consistency evaluation",
    )
    parser.add_argument(
        "--batch_size", type=int, default=8,
        help="Batch size for greedy evalaution"
    )
...
def main():
    ...
    with open(f"{output_dir}/generation_args.json", "w", encoding="utf-8") as write_file:
    ...
    with accelerator.main_process_first():
        test_tokenized_dataset = test_dataset.map(
            lambda x: tokenize_data(x, tokenizer=tokenizer, is_train=False, max_length=max_length),
            batched=True,
    ...
    test_dataloader = DataLoader(test_tokenized_dataset, batch_size=batch_size, shuffle=False, num_workers=8, collate_fn=collator)
    ...
    logger.info(f"  Total test batch size (w. parallel, distributed) = {total_batch_size}")
    ...
    predictions = []
    with torch.no_grad():
        for index, feature in tqdm(enumerate(test_dataloader), disable=not accelerator.is_local_main_process, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', desc="--test", total=len(test_dataloader)):
            ...
            sorted_prediction = [prediction[i * args.num_return_sequences: (i + 1) * args.num_return_sequences] for i in range(len(prediction))]
        ...
        if accelerator.is_main_process:    
            ...
            for samples, metadata in zip(predictions, all_metadata):
                ...
                for sample in samples:
                    try:
                        code, ans, error = runtime.run_code(code_gen=sample, answer_expr="solution()")
                    except:
                        code = ""
                        ans = None
                    try:
                ...
                score = 0
                if final_ans is not None and math.fabs(final_ans - float(metadata["answer"])) < 1e-2:
                    correct += 1
                    score = 1

                all_result.append({"question": metadata["question"],
            ...
            # Store outputs
            with open(f"{output_dir}/test_outputs.json", "a", encoding="utf-8") as write_file:
