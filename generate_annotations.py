...
def parse_args():
    ...
    parser.add_argument(
        '--model', type=str, default="gpt-4-1106-preview",
    ...
    parser.add_argument(
        ...
        help="Top p value used for promting"
    )
    parser.add_argument(
        '--max_tokens', type=int, default=1024, 
        help="Max tokens used for prompting"
    )
    parser.add_argument(
        '--end_problem', type=int, default=-1,
        help="Last problem for generation"
    )
    parser.add_argument(
...
def main():
    ...
    if args.task == "program":
        ...
        with open(path, '+a') as f:
            ...
            progress_bar = tqdm.tqdm(dataset.select(list(range(start_problem, end_problem))) , initial=start_problem, total=end_problem)
            for example in progress_bar:

                question = example['question']
                result = copy.copy(example)
                prompt = UNIT_CONSISTENCY_FEW_SHOT_PROMPT + UNIT_CONSISTENCY_QUESTION_PROMPT.format(question=question)
        ...
        if args.dataset == "gsm8k":
            ...
            for split in dataset.keys():
                ...
                with open(path, "a+") as f:
                    ...
                    progress_bar = tqdm.tqdm(dataset_split.select(list(range(start_problem, end_problem))) , initial=start_problem, total=end_problem)
                    for example in progress_bar:
                        question = example['question']
                        result = copy.copy(example)

                        prompt = CLASSIFICATION_FEW_SHOT_PROMPT.format(question=question)
            ...
            for split in dataset.keys():
                ...
                dataset_split = dataset_split.filter(lambda example: example["Type"].startswith("Common-Division") or example["Type"].startswith("Multiplication"))
