...
def parse_args():
    ...
    parser.add_argument(
        '--synthetic_data_path', required=True,
        help="Synthetic data path to load generated examples"
    )
    parser.add_argument(
        '--output_data_path', required=True,
        help="Dataset name to save as"
    )
...
def main():
    ...
    generated_examples = list(map(json.loads, open(f"synthetic_data/{args.synthetic_data_path}.jsonl")))
    ...
    for generated_example in generated_examples:
        if generated_example["score"] == 1 and \
            "Counter" in generated_example['program'] and \
                "assert " in generated_example['program']:
