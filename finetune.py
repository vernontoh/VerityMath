...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_name", type=str, default="gsm8k_ucp",
        help="Dataset name for finetuning"
    )
    parser.add_argument(
        "--model", type=str, default="codellama/CodeLlama-7b-hf", 
        help="The model to finetune"
    )
    parser.add_argument(
        "--hf_auth_token", type=str, required=True, 
    ...
    parser.add_argument(
        "--warmup_step", type=float, default=-1,
        help="Scheduler warm up steps"
    )
    parser.add_argument(
        "--max_grad_norm", type=float, default=0.3, 
    ...
    parser.add_argument(
        ...
        help="Dataset map num proc"
    )

    # lora config
    parser.add_argument(
        "--lora_r", type=int, default=64,
        help="r value for lora"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Dropout value for lora"
    )
    parser.add_argument(
        "--bias", type=str, default="none",
        help="Bias value for lora"
    )
    ...
    parser.add_argument(
        "--bnb_4bit_use_double_quant", action="store_true", default=False,
...
def main():
    ...
    output_dir = "saved/" + f'{time.strftime("%Y%m%d-%H%M%S")}_{args.model.split("/")[-1]}_{args.num_examples}_{args.dataset_name}'
    if accelerator.is_main_process:
        os.makedirs("saved", exist_ok=True)  
        os.makedirs(output_dir, exist_ok=True)

        # Store all args
        with open(f"{output_dir}/training_args.json", "w", encoding="utf-8") as write_file:
    ...
    with accelerator.main_process_first():
        train_tokenized_dataset = train_dataset.map(
            lambda x: tokenize_data(x, tokenizer=tokenizer, is_train=True, max_length=args.max_length),
            batched=True,
    ...
    train_dataloader = DataLoader(train_tokenized_dataset, batch_size=args.per_device_train_batch_size, shuffle=True, num_workers=8, collate_fn=collator)
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.epsilon, no_deprecation_warning=True)
    ...
    logger.info(f"  Instantaneous batch size per device (training) = {args.per_device_train_batch_size}")
    ...
    for epoch in range(1, args.num_train_epochs+1):
        ...
        for iteration, feature in tqdm(enumerate(train_dataloader, start=1), disable=not accelerator.is_local_main_process, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}', desc="--training batch", total=len(train_dataloader)):
            optimizer.zero_grad()
